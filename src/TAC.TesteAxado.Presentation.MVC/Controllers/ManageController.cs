    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            if (string.IsNullOrEmpty(model.OldPassword))
                ModelState.AddModelError("", "Preencha o campo Senha Atual");

            if (string.IsNullOrEmpty(model.NewPassword))
                ModelState.AddModelError("", "Preencha o campo Nova Senha");
            else if (!model.ConfirmPassword.Equals(model.ConfirmPassword))
                ModelState.AddModelError("", "A confirmação da senha está incorrta, verifique");
